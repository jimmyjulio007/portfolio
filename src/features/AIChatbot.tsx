...
const getInitialMessages = () => {
  ...
  try {
    return JSON.parse(sessionStorage.getItem("chatbot-messages") || "[]");
  } catch {
    return [];
  }
};
...
const markdownComponents = {
  ...
  code: ({ children }: ComponentPropsWithoutRef<"code">) => (
    <code className="bg-white/10 px-1.5 py-0.5 rounded font-mono text-xs break-all">
      {children}
    </code>
  ),
  pre: ({ children }: ComponentPropsWithoutRef<"pre">) => (
    <pre className="bg-white/5 p-3 rounded overflow-x-auto text-xs font-mono mb-2 max-w-full">
      {children}
    </pre>
  ),
  a: ({ href, children }: ComponentPropsWithoutRef<"a">) => (
    <a
      ...
      className="text-[#00f0ff] underline underline-offset-2 break-all"
    >
      {children}
    </a>
  ),
...
function getContextHint(
  ...
  switch (section) {
    case "work":
      return t("hintWork");
    case "process":
      return t("hintProcess");
    case "contact":
      return t("hintContact");
    case "about":
      return t("hintAbout");
    default:
      return null;
...
export function AIChatbot() {
  ...
  const quickActions = useMemo(
    () => [
      {
        label: t("actionProjects"),
        icon: QUICK_ACTION_ICONS[0],
        message: t("actionProjectsMsg"),
      },
      {
        label: t("actionSkills"),
        icon: QUICK_ACTION_ICONS[1],
        message: t("actionSkillsMsg"),
      },
      {
        label: t("actionContact"),
        icon: QUICK_ACTION_ICONS[2],
        message: t("actionContactMsg"),
      },
  ...
  const [reactions, setReactions] = useState<Record<string, "up" | "down">>(
    () => {
      if (typeof window === "undefined") return {};
      try {
        return JSON.parse(sessionStorage.getItem("chatbot-reactions") || "{}");
      } catch {
        return {};
      }
    },
  ...
  const contextHint = useMemo(
    () => getContextHint(visibleSection, t),
    [visibleSection, t],
  );
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: re-run on input change to resize textarea as the user types
  useEffect(() => {
      ...
      try {
        ...
        if (formData.name && formData.email && formData.message) {
          window.dispatchEvent(
            new CustomEvent("fill-contact-form", { detail: formData }),
          );
  ...
  // ─── Auto-scroll ───
  // biome-ignore lint/correctness/useExhaustiveDependencies: re-run to scroll to bottom whenever messages or streaming status change
  useEffect(() => {
  ...
  return (
    ...
    >
      {isOpen && (
        <div
          ...
          className={cn(
            ...
            isMobile
              ? "w-full"
              : "w-[calc(100vw-32px)] sm:w-[350px] md:w-[400px] mb-4",
            isMinimized && "h-[80px]",
          )}
        >
          {/* Cyber Header */}
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-white font-migumono tracking-widest">
                  {t("title")}
                </h3>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-[#00f0ff] font-mono block opacity-70 tracking-tighter uppercase">
                    {t("subtitle")}
                  </span>
              ...
              >
                ...
                {error && (
                  <div className="bg-red-500/5 border border-red-500/20 p-4 rounded-xl flex flex-col items-center gap-3">
                    <p className="text-xs text-red-100 font-mono text-center tracking-tight bg-red-500/10 px-4 py-2 border border-red-500/20">
                      {t("error")}
                    </p>
                    <button
                      onClick={() => regenerate()}
      ...
      {showPulse && !isOpen && (
        // biome-ignore lint/a11y/useSemanticElements: cannot use <button> because it contains a nested dismiss button (invalid interactive nesting)
        <div
          role="button"
          tabIndex={0}
          aria-label={t("title")}
...
const MessageBubble = memo(function MessageBubble({
  ...
  const timestamp = formatTimestamp(
    (message as UIMessage & { createdAt?: Date | string }).createdAt,
  );
      ...
      <div
        className={cn(
          ...
          message.role === "user"
            ...
            : "bg-[#00f0ff]/10 border-[#00f0ff]/20 shadow-[0_0_10px_rgba(0,240,255,0.2)]",
        )}
      >
        {message.role === "user" ? (
